...
- `permissions.deny` の path rule は `Read(path)` / `Edit(path)` のみ有効で、`~/` または `//` anchored 必須 (単一 `/` は settings source 相対、bare name は cwd 相対)。`Write(path)` / `NotebookEdit(path)` / `MultiEdit(path)` / `Glob(path)` は Claude Code が参照しない dead rule。いずれも `validate_permission_gates` が CI で fail-closed に検査する (Issue #303)。secret が directory 単位で増える場所 (`~/.aws` / `~/.ssh` / `~/.config/gcloud` 等) の deny は `~/.aws/**` のように directory glob で書き、ファイル名列挙に頼らない (単一ファイルが secret の `~/.netrc` / `~/.kube/config` 等はファイル指定のままでよい)
