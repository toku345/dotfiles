...
- `codex exec` / `codex login` require `dangerouslyDisableSandbox` — authentication check panics inside sandbox due to macOS `system-configuration` access restriction (Rust `dynamic_store.rs` NULL object panic)
