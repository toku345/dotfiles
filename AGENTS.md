...
- `codex exec` / `codex login` may require `dangerouslyDisableSandbox: true` on **both macOS and Linux**. macOS: authentication checks inside the sandbox hit a `system-configuration` access restriction (Rust `dynamic_store.rs` NULL object panic). Linux: writes fail with `Read-only file system` because `~/.codex` is outside the sandbox write allowlist (probed 2026-09-05). AI automatically judges sandbox bypass; manual disable is rarely necessary
