...
- **Signal handling**: a `trap` on `EXIT`/`INT`/`TERM` sends `SIGTERM` to the three directly-spawned `claude -p` children, then after a `sleep 0.2` (200ms) grace escalates to `SIGKILL`. Descendants (MCP servers, sub-agents, gh/git invocations) inherit termination from their parent `claude` process rather than being signalled directly — this is best-effort and not guaranteed to clean every grandchild in all cases.
