...
`environment` の `**Source control**` が「trusted repo の remote のみ」となっているため、別 `toku345/*` repo は "outside the source control orgs" に該当し、`Data Exfiltration` の soft_deny が発火する（= confirmation prompt）。
