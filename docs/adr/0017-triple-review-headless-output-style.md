...
6. `private_dot_claude/CLAUDE.md` から `headless 起動 (claude -p)` バレット削除
   > **NOTE**: 本ステップは `35945db` (同日 review fix) で部分撤回され、
   > triple-review 非依存の汎用 strict-envelope ガイダンスとして CLAUDE.md に再導入された。
   > 詳細は Status section および [ADR 0019](0019-headless-strict-envelope-general-rule.md) を参照。
