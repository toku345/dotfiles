...
- L728: `/pr-review-toolkit:review-pr` → `pr.md` (background, parallel)
- L732: `/security-review` → `sec.md` (background, parallel)
- L815: aggregator (stdin pipe, wait 完了後に sequential 実行) → `summary.md` (strict envelope: `### 対応必須` 始まり必須)
...
3 spawn site (L728/L732/L815) を全て `claude_p_neutral` 経由に置換。配置は `kill_children` 直後 / `build_aggregation_prompt` 直前。
