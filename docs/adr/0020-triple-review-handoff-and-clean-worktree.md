...
新関数 `require_clean_worktree` を追加。tracked 変更検出で fail-closed:
...
require_clean_worktree() {
  ...
  if [ -n "$dirty" ]; then
    err "Worktree has uncommitted tracked changes:
$dirty
triple-review reviews PR/branch diffs only — uncommitted changes will NOT be reviewed.
...
`--untracked-files=no` 指定で `.gitignore` 対象や build artifact / editor swap file の誤検知を回避し、「commit 漏れ」だけを正確に検知する。
...
- **tracked-only scope**: まだ `git add` していない新規ファイル (untracked) は guard をすり抜ける。この trade-off は `.gitignore` 対象や一時ファイルの誤検知回避と引き換え。意図的に新規追加したコードがレビュー対象から外れる場合、ユーザーが `git add` 後に commit して retry することで自然にカバーされる
...
| ID | 内容 |
|---|---|
| DIRTY-1 | tracked 変更があると `check_prerequisites` 段で err 中断 (exit 1) |
| DIRTY-2 | untracked のみの状態では guard を通過 (= `--untracked-files=no` の効果検証) |
| DIRTY-3 | `git status` 自体が失敗する病的ケースで err 中断 (`|| err` の動作確認) |
| HANDOFF-1 | `exec claude --` の handoff payload に 3 ファイルパスが含まれる (`claude` stub による argv capture) |
