...
1. **Slim SKILL.md** — concentrate HARD-GATE, Pre-send self-check, and core rules at the top of a tightly skeletoned file so the highest-priority rules retain salience at every checkpoint.
...
- **Line budget is a proxy, not a guarantee**: the structural bats gate enforces a line ceiling, but always-loaded text density (compound rules within a single line, escape hatches inside core rules) can still erode salience while the line count stays green. Line count is a coarse proxy for salience; runtime adherence still requires fresh-session evaluation and the prompt fixtures in `tests/skills/brainstorming/prompts/`.
