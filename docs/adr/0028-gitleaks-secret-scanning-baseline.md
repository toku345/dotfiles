...
- **L3 — server-side push protection.** GitHub secret scanning + push protection, enabled per public repo, is the layer that survives a bypassed local hook. Repository push protection is still bypassable by users with the relevant privileges unless delegated bypass or equivalent policy controls are configured, so required CI remains the fail-closed merge gate.
...
- gitleaks is in maintenance mode (security patches only) as of 2026-06, with its maintainer focusing on the successor `betterleaks` (same team; not drop-in compatible — different CLI, `.betterleaks.toml`). gitleaks remains stable and security-patched, so there is no near-term breakage risk; migration is tracked in #259.
