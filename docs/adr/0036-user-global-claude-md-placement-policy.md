...
**Amendment 2026-08-26**: Context §3 で load-bearing と記録した `permissions.ask` エントリと、Decision に基づく `permissions.allow` からの `Bash(codex exec:*)` 削除は、`tests/codex/verify_claude_update_policy.py` の `validate_permission_gates` が CI で pin するようになった。記録した時点では enforcement が無く、`permissions` ブロックへの reformat / merge 解決 / 同種の外科的整理で当該エントリが silently 落ちても何も fail しない状態だった (損失が観測されるのは auto-mode 下で main への無断 push が通った瞬間)。判定は exact-list 一致ではなく required-subset / forbidden-entry で行うため、エントリの正当な追加ではテストは壊れず、列挙した load-bearing エントリを削除したときだけ fail する。`Bash(codex exec:*)` の forbidden 判定だけは一般原則 (narrow shell allow rule は分類器より先に解決する) ではなく本 ADR の個別決定を pin するものであり、将来 allow へ戻す場合はテスト側の編集という意識的な判断を要求する。pin はエントリの存在だけでなく `permissions.defaultMode` が `bypassPermissions` でないことも要求する — エントリが揃っていても bypass 既定なら prompt が一切上がらず全 ask が inert になるため。negative test 側は tuple の期待内容を literal で列挙して完全一致を assert し、CLI subprocess case で `main()` の合成も pin する: 生成 case だけに頼ると tuple からエントリを消したときに case も一緒に消え、`main()` から validator の呼び出しを外しても実 settings が compliant なため両 CI コマンドが緑のまま通ってしまう (いずれも実機で再現を確認した)。本 pin の限界は 3 つ — tuple と test の期待値を両方編集すればエントリは除去できる (silent な reformat / merge 解決を防ぐのが目的であり、除去自体の禁止ではない)、`--dangerously-skip-permissions` CLI flag は `permissions.disableBypassPermissionsMode` でしか塞げず本 pin の対象外、検証対象は chezmoi source (`private_dot_claude/settings.json`) であって live `~/.claude/settings.json` の drift は `chezmoi diff` が担う。 同じ validator は 2026-09-02 から `permissions.deny` の構造不変条件も検査する: `Read(path)` / `Edit(path)` は `~/` または `//` anchored でなければならない (単一 `/` は settings source 相対、bare name は cwd 相対で、user-global では守るべき path を守らず、sandbox の boundary merge 経由で ADR 0001 / Issue #212 の `chezmoi apply` 失敗を再発させる)、および `Write` / `NotebookEdit` / `MultiEdit` / `Glob` の path rule は Claude Code が参照しない dead rule として禁止する (公式 docs: file permission は `Read(path)` / `Edit(path)` のみ照合、他は startup warning のみ)。path を持たない tool-level deny (`NotebookEdit` 等) は対象外。
