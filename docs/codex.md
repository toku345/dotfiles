...
4. baseline を更新したら、`chezmoi apply` が drift を検出して exit 1 で停止する。`diff -u ~/.codex/config.toml ~/.codex/config.chezmoi.toml` で差分を確認し、local-only section を保ったまま baseline 更新分を live に取り込んで再 `chezmoi apply` する。無関係な dotfile を急ぎ apply したい場合は `chezmoi apply <target>` で個別指定すればこの script は trigger されない
