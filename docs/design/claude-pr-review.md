...
- **Built-in review parity trigger:** retire (or shrink) this gate entirely if Claude Code's built-in review skills (`/code-review`, `/security-review`) grow to cover the gate requirements: the specialist breadth beyond correctness/security (test-gap, silent-failure, comment-accuracy, type-design, adversarial — the legs that produced most findings in the live runs), a user-owned severity/caps policy shared with the Codex-side skill, pinned-scope fail-closed coverage guarantees, and a verification stage for high-severity findings. Until then the division of labor stands: built-in review for everyday changes (per `review-criteria.md` Quick review), this gate for thick pre-merge changes only. Reassess when built-in review capabilities change materially.
