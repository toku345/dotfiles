...
- The operator alone creates and modifies each draft contract and its Checkpoint 1 approval receipt. The approved contract digest is recorded in the scorecard, the whole contract becomes read-only with no writer, and any required contract change creates a new run rather than editing it in place.
- Before any rehearsal or real task, each runtime must demonstrate a default-deny runtime/OS sandbox or approval profile with explicit read and write roots, credential/environment/socket exposure, and network enforcement. The agent may read the target worktree and approved contract; it may write only approved target-worktree paths, the run output directory, and declared disposable temporary/cache paths. Git metadata, other repositories, operator-owned pilot state, global configuration, tool-readable credentials or secret-bearing inherited environment variables, credential/keychain/agent sockets, persistent services/databases, and external write-capable network access remain unavailable. Tool-visible network is disabled unless enforcement restricts destinations and request shapes to an approved read-only allowlist; otherwise the operator supplies primary-source material or fixtures.
...
- Sandbox negative controls use disposable fixtures and harmless sentinels: a write outside an allowed root, a Git-metadata mutation in a fixture repository, an operator-artifact write, a credential-source read, an undeclared-egress request, and a mock external-state write must all be denied before mutation or disclosure. The launch record also confirms that secret-bearing environment variables and credential/keychain/agent sockets are unavailable to role tools without recording secret values. Post-run inspection remains defense in depth, not the primary control. If either runtime cannot demonstrate denial, that pilot arm is blocked.
- Passing controls are role-specific for `A1-discovery`, `A2-blind-spot`, `B-implementation`, `C-review`, and the optional `spike-temp` exception. Every record includes runtime, OS, sandbox/approval-profile identity and configuration digest, allowed read/write roots, credential/environment/socket exposure, network mode plus destination/request-shape allowlist digests when enabled, and invocation version. Before every session, the operator compares that role's current enforcement identity and boundaries with its last passing record. Any drift reruns the role's negative controls and, when it changes runtime behavior, end-to-end rehearsal and calibration; the session remains blocked until they pass.
...
- schema version, package digest, task, run, runtime/model, invocation version, enforcement-profile identity/configuration digest, approved read/write roots, credential/environment/socket exposure, network mode and allowlist digests when enabled, provenance, and human-approved authority
- Goal and Non-goals
...
- all run ids, contract/report/diff/Evidence Packet digests, recomputed package digest, driver/reviewer runtime and model, same-model/cross-model review mode, and role-specific enforcement-profile identity/configuration digests, approved roots, credential/environment/socket exposure, network modes and allowlist digests, and passing-control record ids
...
3. Run role-specific safe sandbox negative controls for A1, A2, B, C, and spike-temp against disposable sentinels and a fixture repository, including credential-source reads and undeclared egress; confirm secret-bearing environment variables and credential/keychain/agent sockets are unavailable to role tools, and block the Private arm if any prohibited write, read, or disclosure is allowed or only detected afterward.
...
7. Exercise simulated safety and gate fixtures without performing a real prohibited operation: an attempted fixture Git mutation, suppressed required stop, unverified pass claim, unresolved quiz miss, failed or unverified Acceptance Criterion, blocked or unresolved Unknown, unresolved queued decision, incomplete Session C review, and unresolved `blocks-ship` finding. The first three are hard-failure fixtures; the remaining failed gates reject `ship` and require `narrow`, `redirect`, or `block`, while suppressing or ignoring a required stop is a hard failure.
...
10. Recompute the package digest against the last calibrated digest, compare the C-role enforcement identity, roots, credential/environment/socket exposure, and network enforcement with its passing control, and rerun controls on any drift, then run fresh blind-first Session C with read-only inputs. The operator records durable evidence of the fresh context and Goal/AC/diff/verification-before-Decision-Log order, plus findings, understanding questions, and every reviewer-discovered Unknown with the full mapping/routing/evidence/owner fields; Session C writes no worktree or pilot artifact.
11. Present the frozen packet and quiz to start the CP2 timer. Resolve every quiz miss from evidence within at most two answer rounds and record `ship` only when the full ship gate passes, including Session C freshness and blind-first evidence; otherwise record `narrow`, `redirect`, or `block`. Unrelated interruptions pause the timer, 20 minutes is the qualification threshold, and 30 minutes without disposition forces `block`.
...
| Sandbox enforcement | A1/A2/B/C/spike role roots, credential/environment/socket exposure, network allowlists, and profile/config digests match their last passing controls before every session | Outside-root/Git/operator-artifact writes, credential-sentinel reads, undeclared egress, and mock external writes are denied before mutation or disclosure; any boundary drift reruns controls and blocks the session |
| Independent review | Scorecard proves fresh Session C context and Goal/AC/diff/verification review before the Decision Log | Missing or failed freshness/order evidence blocks `ship` |
