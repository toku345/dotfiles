...
- The CP1-approved contract digest is recorded in the scorecard. The approved contract becomes read-only, and any required contract change creates a new run rather than editing it in place.
- Before any rehearsal or real task, each runtime must demonstrate a default-deny runtime/OS sandbox or approval profile with explicit read and write roots. The agent may read the target worktree and approved contract; it may write only approved target-worktree paths, the run output directory, and declared disposable temporary/cache paths. Git metadata, other repositories, operator-owned pilot state, global configuration, credentials, persistent services/databases, and external write-capable network access remain outside its readable/writable roots. If read-only network behavior cannot be distinguished from write-capable access, network access is disabled and the operator supplies primary-source material or fixtures.
- During Session B, `report.md` is the only pilot artifact the `/goal` agent may modify. Contract, scorecard, company-arm summary, cohort decision, generalized-learning records, and package manifest are operator-owned and outside the agent's write roots. A permission request to cross this boundary is rejected rather than used as a Week 0 override.
- Sandbox negative controls use disposable fixtures: a write outside an allowed root, a Git-metadata mutation in a fixture repository, an operator-artifact write, and a mock external-state write must all be denied before the pilot arm starts. Post-run inspection remains defense in depth, not the primary control. If either runtime cannot demonstrate denial, that pilot arm is blocked.
- Passing controls are role-specific for `A1-discovery`, `A2-blind-spot`, `B-implementation`, `C-review`, and the optional `spike-temp` exception. Every record includes runtime, OS, sandbox/approval-profile identity and configuration digest, allowed read/write roots, network mode, and invocation version. Before every session, the operator compares that role's current enforcement identity and roots with its last passing record. Any drift reruns the role's negative controls and, when it changes runtime behavior, end-to-end rehearsal and calibration; the session remains blocked until they pass.
...
- Session B is fresh. It creates or resumes the runtime's goal using the approved read-only contract as its sole task-specific instruction source; repository and global safety guidance still applies. It records a contract echo-back in `report.md` and stops at `CP2_READY`; runtime self-declared completion is not verification evidence. At `CP2_READY`, the operator stops Session B, freezes the report read-only, and records report, diff, and Evidence Packet digests in the task scorecard before review starts.
- Session C is fresh and stays inside the same information boundary. It receives read-only snapshots of Goal, Acceptance Criteria, contract, diff, report, and verification. It reviews Goal, Acceptance Criteria, diff, and verification before reading the driver Decision Log, then returns findings and three to five understanding questions without modifying the worktree or pilot artifacts. The operator records that output in the scorecard's CP2 section.
...
- A same-intent `narrow` or `redirect` returns through bounded discovery and Checkpoint 1, then creates a new run and a new Session B under the same task id. A material Goal replacement ends the original task as non-qualifying and starts a new task id.
...
- schema version, package digest, task, run, runtime/model, invocation version, enforcement-profile identity/configuration digest, approved read/write roots and network mode, provenance, and human-approved authority
- Goal and Non-goals
- Constraints and invariants
- observable Acceptance Criteria with a verification method for each
- up to five high-impact Unknown candidates with affected behavior/interface, false-assumption failure, related Acceptance Criterion, primary evidence, cheapest falsification probe, route/status, resolution evidence, and accepted-risk owner when applicable; every candidate must be routed before CP1 approval
...
- every implementation-time Unknown with affected behavior/interface, false-assumption failure, related Acceptance Criterion/invariant/authority boundary, current evidence and cheapest probe, route/status, resolution evidence, and accepted-risk owner when applicable
...
Every implementation-time or reviewer-discovered Unknown must have complete mapping, routing, evidence, and ownership before `ship`. A missing route or resolution record is `UNVERIFIED` and blocks the quiz gate and disposition.
...
- a self-contained durable summary of Goal, Non-goals, Acceptance Criteria, invariants, major implementation decisions, queued-decision outcomes, all Unknown dispositions, key evidence and provenance, residual risk, rollback, and final behavior
- all run ids, contract/report/diff/Evidence Packet digests, recomputed package digest, driver/reviewer runtime and model, same-model/cross-model review mode, and role-specific enforcement-profile identity/configuration digests, approved roots/network modes, and passing-control record ids
...
It contains no task/run/repository identifier, order, date, timestamp, path, raw metric, context-linked failure reason, or reconstructable combination. It crosses the boundary only after human review and only through a company-permitted path. Otherwise the human performs the final comparison in place.
...
- human abstraction/reconstructability decision

It contains no Work task/run link, timing association, repository detail, internal convention, Skill/prompt detail, raw measurement, or context-specific reason. Ambiguous statements remain on Work.

### `cohort.md` Private decision record

The template has two mutually exclusive modes:

- `approved-summary`: record schema/package identity, the two local Private task ids, the approved company-arm summary, combined all-hard-gates result, total comfort-qualifying count, Codex/Claude `ship` coverage, prior-cohort observations, and the human-approved `advance`, `revise and rerun`, or `stop` decision.
- `in-place-no-transfer`: record only the schema/package identity, two local Private task results, `work_comparison: completed_in_place`, and an optional final generalized directive that company policy explicitly permits to cross. A permitted directive is formally reviewed and recorded as the only cross-boundary output. Work-derived counts, coverage, gate results, reasons, and combined evidence remain outside Private. If even the directive is not permitted, Private remains `awaiting_external_decision` and cannot start or claim the shared Skill phase; any further work stays Work-local or waits for an approved channel.
...
- Each environment maintains an operator-owned local `screening.md` in candidate-arrival order, including every considered candidate, eligibility result, and exclusion reason. Excluded candidates are not silently omitted.
...
- A task may have at most two implementation runs, allowing one same-intent re-gate. If the second run cannot reach terminal disposition, the task ends as `block` or abandonment and remains non-qualifying in the cohort.
...
3. Run role-specific safe sandbox negative controls for A1, A2, B, C, and spike-temp against disposable sentinels and a fixture repository; block the Private arm if any prohibited write is allowed or only detected afterward.
...
5. Verify pre-CP1 worktree read-only behavior, directory/file permissions, enforcement identity recording and drift detection, contract/report/diff/Evidence digests, contract and frozen-report immutability, raw-artifact cleanup, and the self-contained scorecard's independence from deleted temp files.
6. Exercise same-intent redirect accumulation and material-Goal-replacement accounting.
7. Exercise simulated hard-failure fixtures without performing a real prohibited operation: an attempted fixture Git mutation, suppressed required stop, unverified pass claim, and unresolved quiz miss.
8. Confirm the fixture and target worktree HEAD, index, refs, stash, branch/worktree metadata, global config, credentials, services, databases, and external state remain unchanged.

Done when:

- All fake-run measurements roll up to one task without losing prior-run cost.
- Every hard-failure fixture pauses the fake pilot.
- Every prohibited-write negative control is denied before mutation, not merely found by post-run inspection.
...
7. On each runtime, rehearse one same-task redirect path, including frozen report/diff/evidence digests, the calibrated goal close/wait behavior, a new run under the same task id, cumulative metrics, and the two-run terminal bound.
8. Rehearse app/terminal restart or goal-resume failure and verify it is recovered as a new run without losing prior task evidence.
...
The cohort is fixed to two eligible Codex tasks and two eligible Claude Code tasks under one schema version. Order may follow when eligible work appears; tasks are not substituted after a bad result.

For each task:

1. Append the candidate to the environment-local screening log in arrival order, apply the eligibility/exclusion checklist, and record either the exclusion reason or the eligible task id.
...
8. Reach `CP2_READY` only with complete Acceptance-Criterion evidence, queued decisions, risk, rollback, time/turn compliance, and explicit `UNVERIFIED` entries.
9. At `CP2_READY`, stop Session B, freeze `report.md`, snapshot diff and Evidence Packet, and have the operator record all three digests plus the durable Goal/AC/invariant, decision, Unknown, evidence/provenance, residual-risk, rollback, and final-behavior summaries in the scorecard. A post-freeze fix always uses a new run in Week 0; the frozen report is never reopened.
10. Recompute the package digest against the last calibrated digest, compare the C-role enforcement identity/roots/network mode with its passing control, and rerun controls on any drift, then run fresh blind-first Session C with read-only inputs. The operator records its findings, understanding questions, and any reviewer-discovered Unknown with the full mapping/routing/evidence/owner fields in the scorecard; Session C writes no worktree or pilot artifact.
11. Present the frozen packet and quiz to start the CP2 timer. Resolve every quiz miss from evidence within at most two answer rounds and record `ship`, `narrow`, `redirect`, or `block` in the scorecard; unrelated interruptions pause the timer, 20 minutes is the qualification threshold, and 30 minutes without disposition forces `block`.
12. A same-intent `narrow`, `redirect`, or post-freeze fix loops through a new CP1-approved contract and run under the existing task id, subject to the two-run maximum. Roll all run metrics and failures into the task scorecard; a new run never resets the baseline or cumulative values.
...
4. Choose exactly one cohort-record mode. In `approved-summary` mode, transfer the allowlisted summary through a permitted path and compute the combined decision on Private. In `in-place-no-transfer` mode, compare the machines in place and do not copy Work-derived counts, coverage, gates, or combined evidence into Private.
...
8. Record `advance`, `revise and rerun`, or `stop` in the location permitted by the selected cohort mode. If only a final generalized directive may cross, formally review and record it as the allowed output without underlying Work-derived values. If no directive may cross, Private remains `awaiting_external_decision` and the shared Skill phase cannot start there.
...
| Candidate/baseline | Every candidate and fixed class/scope/recency match includes historical attention and confidence before CP1 | Silent exclusion, unmatched estimate counted as qualifying, or post-outcome replacement invalidates the task |
...
| Contract | Schema/package/contract identity remains unchanged for one run and every pre-/in-/post-implementation Unknown has mapping, route, evidence, and owner | Required contract edit creates a new run; any unrouted or unsupported Unknown blocks CP1 or `ship` |
...
| Report ownership | Session B report freezes at `CP2_READY`; Session C is read-only; operator records CP2 evidence | Report/diff/evidence mutation after freeze is a hard integrity failure |
...
| Task bound | One implementation run or one permitted re-gate reaches terminal disposition within CP1/quiz/CP2 limits | Third run, third quiz round, or unresolved CP1/CP2 hard-cap overrun ends `block`/abandonment |
| Sandbox enforcement | A1/A2/B/C/spike role roots and profile/config digests match their last passing controls before every session | Safe negative controls must be denied before mutation; any role identity/root drift reruns controls and blocks the session |
...
| Information boundary | One allowlisted Work arm summary passes human review | Run/task ids, timing, raw metrics, linked reason, or reconstructable content stays local |
...
| No-transfer mode leaks Work results through combined values | Keep Work-derived counts/evidence out of Private and allow only a policy-approved final generalized directive |
...
| Checkpoint loops recreate full-time supervision | Fixed CP1 presentations/time, quiz rounds, CP2 hard cap, and two-run task maximum |
