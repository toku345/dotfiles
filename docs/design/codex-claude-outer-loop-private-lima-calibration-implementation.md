...
2. `preflight` validates the source lock and manifest, freezes the manifest-listed harness, validates the frozen manifest again, and then checks the frozen Codex `0.144.5` lock plus seed contract before H1 or freshness evidence. The normalized requirements allow exactly `never`, `read-only` plus `workspace-write`, and `disabled`, while the active config selects `never`, `workspace-write`, and `disabled`. It also validates the parser, host and physical-home identities, exact objective/prohibitions, terminal states, and risk records `AR-01` and `AR-02`. It parses Lima `2.2.0` output as JSON Lines and requires an empty namespace plus both fixed instance-directory and root-disk paths absent. JSON arrays, malformed or mistyped records, `Broken` instance status, additional/mixed stderr, nonzero status, and non-canonical empty output are `UNKNOWN` and block.
...
Policy validation has two stages. Before H1, the host validates the already frozen lock and seed files against the known Codex `0.144.5` contract, so a missing `read-only` permission profile or any extra, duplicate, mistyped, or drifted value blocks before VM creation. This static validator deliberately encodes only the reviewed version-specific invariant. After start, the pinned Codex binary runs as the non-sudo `calibration` user with the guest-local `CODEX_HOME`; its `app-server --strict-config` handshake and key-by-key effective-policy read-back remain authoritative.
...
CI and local verification run Python `unittest` on Python `3.14.6`, ShellCheck on guest shell, and `limactl validate --tty=false` for both profiles on macOS. CI never starts a VM. Tests use temporary logical and physical roots and cover C00-C08 model contracts, lock/manifest drift, the exact Codex `0.144.5` frozen seed contract and its pre-freshness failure ordering, strict zero/one/multiple-object JSON Lines parsing, `Broken` instance rejection, socket-path budget, logical-first allocation, run-bound pool propagation, preflight/pre-create absence, explicit create/Stopped/start/Running ordering, calibration-user runtime identity checks, the exact cidata mount exception and neighboring negative paths, sanitized fixed failure-stage reporting, non-success/orphan provision attempts, risk/control separation, sync/export negative shapes, retention no-retry/manual disable, never-provisioned empty-home verification, administrative-residue manual routing, read-only status diagnostics, sanitizer leakage, runtime-schema-1 immutability, and the immutable historical paths.
