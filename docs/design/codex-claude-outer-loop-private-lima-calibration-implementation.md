...
- Lima `2.2.0`, the Homebrew arm64 bottle digest, and the expected host `limactl` digest.
- Python `3.14.6` and the expected host interpreter digest.
...
2. `preflight` validates the source lock and manifest, freezes the manifest-listed harness, validates the frozen manifest again, and then checks the reviewed policy subset and pinned artifact versions in the frozen Codex `0.144.5` lock and seeds before H1 or freshness evidence. The normalized requirements allow exactly `never`, `read-only` plus `workspace-write`, and `disabled`, while the active config selects `never`, `workspace-write`, and `disabled`. It also validates the parser, host and physical-home identities, exact objective/prohibitions, terminal states, and risk records `AR-01` and `AR-02`. It parses Lima `2.2.0` output as JSON Lines and requires an empty namespace plus both fixed instance-directory and root-disk paths absent. JSON arrays, malformed or mistyped records, `Broken` instance status, additional/mixed stderr, nonzero status, and non-canonical empty output are `UNKNOWN` and block.
...
Policy validation has two stages. Before H1, the host checks three active policy keys, their three allowlists, and two pinned artifact versions in the already frozen Codex `0.144.5` lock and seeds. A missing or mistyped active value, a missing, extra, duplicate, or mistyped allowlist value, or artifact-version drift blocks before VM creation. Other frozen seed bytes are bound by the manifest rather than semantically interpreted by this static validator. After start, the pinned Codex binary runs as the non-sudo `calibration` user with the guest-local `CODEX_HOME`; its `app-server --strict-config` handshake and key-by-key effective-policy read-back remain authoritative.

C00 starts app-server over stdio, waits for the initialize result before sending `initialized`, and waits for each read result before issuing the next request. It calls `config/read` with `includeLayers: true` and a fixed harmless cwd plus `configRequirements/read`. Every flattened seed key must map to the expected effective value and expected system/requirements origin. For list-valued config, every indexed origin must be present and resolve to the same expected system policy file; missing or mixed indexed provenance fails closed. Missing keys, unknown seed keys, origin drift, incompatible effective values, or cloud-composed requirements mismatches fail. A digest without key-level comparison cannot pass.
...
CI and local verification run Python `unittest` on Python `3.14.6`, ShellCheck on guest shell, and `limactl validate --tty=false` for both profiles on macOS. CI never starts a VM. Tests use temporary logical and physical roots and cover C00-C08 model contracts, lock/manifest drift, the reviewed Codex `0.144.5` policy subset and pinned artifact versions plus their pre-freshness failure ordering, strict zero/one/multiple-object JSON Lines parsing, `Broken` instance rejection, socket-path budget, logical-first allocation, run-bound pool propagation, preflight/pre-create absence, explicit create/Stopped/start/Running ordering, calibration-user runtime identity checks, the exact cidata mount exception and neighboring negative paths, sanitized fixed failure-stage reporting, non-success/orphan provision attempts, risk/control separation, sync/export negative shapes, retention no-retry/manual disable, never-provisioned empty-home verification, administrative-residue manual routing, read-only status diagnostics, sanitizer leakage, runtime-schema-1 immutability, and the immutable historical paths.
