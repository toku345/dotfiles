...
Status: Draft (2026-08-30, rev.15 — one-shot verifier comparison)
...
Current verification boundary: CI checks bundle integrity and static contract drift via `tests/codex/verify_pr_review_bundle.py`, proves selected high-risk verifier regressions fail closed with `tests/codex/test_verify_pr_review_bundle_negative.py`, validates the accepted historical baseline, sanitized post-change comparison, and finding-verdict contract, replays PR and `--allow-no-pr` base-resolution command/control-plane decisions with `tests/codex/test_pr_review_base_resolution.py`, and executes the V2 abnormal-path state-machine plus retained-list and scheduler-owned dispatch replays with `tests/codex/test_pr_review_v2_scheduler.py`. Static checks cover the three `gpt-5.6-sol` review profiles, both runtime schemas, all pinned machine-readable contracts, agent control-plane bans, first-party read-only inheritance, severity normalization, `$HEAD_REF`-bound collection, authoritative packet hashing, exact candidate identity, and negative mutations for high-risk drift. Deterministic replay covers scoped escalation, Stage 1/2/3 lifecycle boundaries, scheduler refill, cleanup ownership, exact-role/candidate completeness, verdict shape, and aggregation decisions. CI does **not** prove Codex CLI profile layering or run live model requests; isolated manual smoke records the effective turn context and actual tool calls without retaining Raw JSONL.
...
| **Orchestrator or verifier bug → false-green or false-red** — candidate extraction, identity matching, or verdict application could hide a valid blocker or retain a false positive | Low–Medium | The accepted historical fixtures preserve pre/post observations without inventing a hard recall threshold. Static and executable tests pin deterministic IDs, one-candidate scope, exact result-set completeness, verdict shapes, visible refutations, and Critical downgrade rules; post-change comparison remains observational because effective model identity was not exposed in every run |
