...
Status: Draft (2026-07-24, rev.12 — V2 retained-task retirement compatibility)
...
The skill reads `references/base-resolution-runtime-contract.json` for both runtime adapters. Every allowlisted network or Git-metadata operation runs sandbox-first. A proven sandbox/transport denial may retry the exact original executable, argv, cwd, and ordinary environment once with `sandbox_permissions=require_escalated`; approval metadata and that tool field are the only permitted fingerprint differences. The allowlist is `gh pr view`, validated-branch/default fetch, and `git remote set-head origin --auto`. Persistent prefix approvals, blanket network enablement, and `danger-full-access` are forbidden.
...
- A task is usable only after a valid authoritative final payload and accepted lifecycle evidence. Normal evidence is completed status. The V2-only compatibility fallback is qualified retirement: the exact canonical path was observed running in a successful full-tree snapshot, the parent never interrupted it, no run-global fatal exists, and a later successful full-tree snapshot omits it. This relies on the recognized V2 contract delivering `FINAL_ANSWER` at child-turn completion; unknown schema semantics fail closed.
...
- Ignore interim `MESSAGE` payloads; they do not reset any timeout. Ignore byte-identical duplicate finals, but abort on conflicting duplicate finals, error/interrupted status, unobserved or incomplete-snapshot disappearance, parent-interrupted retirement, a name/path collision, or any unexpected descendant.
...
| retired-seen | observed-running canonical task is absent from a later successful full-tree snapshot, final not delivered | enter the same bounded delivery grace |
...
| fatal | timeout, conflict, error/interrupted, unobserved disappearance, invalid retirement/coverage, or ambiguous spawn | owned cleanup and fail closed |
...
- [x] **Base-resolution abnormal paths covered deterministically**: `PR_REVIEW_BASE_RESOLUTION_CONTRACT_V1` and `tests/codex/test_pr_review_base_resolution.py` replay sanitized sandbox denial, elevated PR metadata, fetch EROFS, elevated fetch, and OID equality; credential failure, approval denial/unavailable, ordinary ref failure, retry/fingerprint drift, malformed base evidence, and immutable-OID zero-escalation fail closed before specialist spawn.
...
- [x] **V2 abnormal-path executable contract coverage**: the distributed Skill reads pinned scheduler contract V2; `tests/codex/verify_pr_review_bundle.py` rejects drift, and `tests/codex/test_pr_review_v2_scheduler.py` consumes the same file while parameterizing Stage 1/2 deadlines and exercising completed/retirement-first delivery inside and at the 60-second boundary, observed-running and full-tree requirements, parent interruption, identical/conflicting finals before aggregation, later error/interrupted evidence, cleanup monotonicity, canonical sender matching, spawn reconciliation, unexpected descendants, owned cleanup, exact-role aggregation, and partial-aggregation rejection.
...
Current verification boundary: CI checks bundle integrity and static contract drift via `tests/codex/verify_pr_review_bundle.py`, proves selected high-risk verifier regressions fail closed with `tests/codex/test_verify_pr_review_bundle_negative.py`, replays base-resolution command/control-plane decisions with `tests/codex/test_pr_review_base_resolution.py`, and executes the V2 abnormal-path state-machine plus retained-list run replay with `tests/codex/test_pr_review_v2_scheduler.py`. Static checks cover the three `gpt-5.6-sol` review profiles, their inherited approval/network/proxy baseline, rejection of legacy profile tables and `hide_spawn_agent_metadata`, both runtime schemas, both pinned machine-readable contracts, specialist control-plane bans, severity normalization, fatal-on-missing-context wording, `$HEAD_REF`-bound collection, authoritative diff-packet hash requirements, and negative mutations for high-risk drift. Deterministic replay covers scoped escalation, retry/fingerprint limits, OID verification, Stage 1/2 lifecycle ordering and grace boundaries, qualified retirement, slot-bounded refill, cleanup ownership, exact-role completeness, and aggregation decisions. CI does **not** prove Codex CLI profile layering or run live model requests; isolated manual smoke records the effective turn context and actual tool calls without retaining Raw JSONL.
...
| **V2 retained-list rollover hides completed status** — final delivery, slot refill, and status retention can race; requiring explicit completed status alone creates false-negative gates | Low–Medium | Contract V2 requires canonical FINAL plus completed status or qualified retirement after observed running in successful full-tree snapshots. It preserves strict 60-second/stage boundaries, parent-interrupt and run-fatal exclusions, ordered reconciliation, late-conflict validation, and exact-role aggregation. Sanitized run-level replay pins the observed sequence |
