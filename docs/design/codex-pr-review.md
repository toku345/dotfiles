...
Current quality work: [Issue #282](https://github.com/toku345/dotfiles/issues/282)
Status: Draft (2026-08-31, rev.17 — immutable citation validation)
...
- **Maintainability**: replace 1144 lines of bash with one declarative Skill, 8 review specialist `.toml` agents, 1 first-party finding verifier, 3 LICENSE files, and 1 NOTICE file while keeping runtime adaptation and gate invariants explicit
...
| **α** | **Pure Codex Skill + specialist review and evidence verification subagents** | **✅** | intended successor to bash `triple-review`; credit-neutral; narrow context per task; standard OSS licensing |
...
16. **Finding verification (rev.14)**: Issue #282 adds a first-party read-only `finding-verifier` after normalization and optional simplification. Each deterministic Critical/Important candidate gets one isolated task that may only confirm, refute, or name exact missing verification; it cannot discover findings or choose severity. Scheduler contract `PR_REVIEW_V2_SCHEDULER_CONTRACT_V4` adds the 30-minute Stage 3 budget and exact candidate-ID aggregation. Refuted candidates remain visible outside the fix queue, `needs-verification` downgrades only Critical candidates, and malformed, missing, duplicate, or partial verifier results fail closed.
17. **One-shot historical comparison (rev.15)**: the isolated candidate bundle was run exactly once, without retries, on buggy/fixed snapshots from PRs #304, #330, and #269. Four gates completed and two failed closed. The completed gates sent 21 candidates through Stage 3: 18 were confirmed, 2 needed verification, and 1 was refuted. Total usage was 16,675,099 input tokens, of which 16,107,520 were cached. The two completed buggy cases did not reproduce their pre-change known roots, while the third buggy case aborted before specialist spawn; because candidate creation happens in Stage 1 and effective model identity was unverified, this cannot be attributed to Stage 3 or scored as a controlled recall regression. It also does not demonstrate a controlled false-positive improvement. The sanitized `PR_REVIEW_FINDING_VERIFIER_POST_V1` fixture preserves this negative/ambiguous result so Draft-PR review can judge the cost and evidence without rerunning for finding quality.
18. **Post-gate verifier hardening (rev.16)**: candidate gate session `01a0500f-b0db-7830-9f55-e30879042f4d` completed all seven Stage-1 roles, Stage 2, and five Stage-3 verifiers against immutable base `72daee96446a51158b39eb7c9383853f96ca48dd`; all five verifier candidates were confirmed. Follow-up closes the deterministic gaps: verdict application now has executable mixed/zero/partial/capped aggregation coverage, V1 Stage 3 replays returned-agent identity plus targeted wait/close/refill/deadline/error cleanup, comparison validation requires per-case verdict accounting, and evidence values require repository-relative paths, positive lines/ranges, and non-empty observations. Historical artifact hashes remain pinned to the evaluated candidate instead of being compared with the mutable current checkout. The requested hard recall threshold was not adopted: the one-shot runs lack controlled model identity and include aborts, so the evidence cannot justify a numeric threshold. The `0/2` evaluable known-root observation remains visible and merge readiness remains a human decision.
19. **Immutable citation validation (rev.17)**: PR #347 review demonstrated that syntactically valid but nonexistent or out-of-bounds citations could still drive `confirmed` or `refuted`, including removal of a real blocker from the fix queue. Stage 3 now serializes each parsed verifier JSON object outside the worktree and requires the bundled deterministic validator to resolve every citation as a literal regular text blob in the exact recorded `$HEAD_REF` tree, with replacement objects disabled and the cited line/range bounded by the immutable blob's logical line count. Missing, non-file, binary, control-character, or out-of-bounds evidence fails closed before verdict application; current worktree contents are never evidence. CI exercises valid citations plus nonexistent, worktree-only, symlink/tree, binary, empty, and immutable-boundary failures.
...
├── finding-verifier.toml                  # MIT (first-party)
...
- Historical comparison evidence additionally records one-shot status, normalized counts, verifier verdict counts, aggregate token usage, and whether a predeclared known root appeared. Missing model identity and aborted cases remain explicitly unscored; the fixture must not claim causal quality improvement or regression.
...
**Sequential review, simplification, and verification**:

The orchestrator implements a 3-stage flow: run the Stage-1 review set and require complete valid coverage; normalize and deduplicate Critical/Important candidates; run advisory-only Stage-2 `code-simplifier` when Stage 1 has no Critical findings; then run one Stage-3 `finding-verifier` task per recorded candidate. Stage 2 cannot add candidates and is not repeated after verification. Both runtime adapters preserve the same immutable scope, packet hash, deadline, read-only, result-integrity, and fail-closed completeness rules. In V2, all three stages use matching canonical `FINAL_ANSWER`, completed-or-qualified-retirement lifecycle evidence, delivery grace, and ordered reconciliation. Stage 3 additionally requires the exact candidate-ID set, a contract-valid JSON verdict for every candidate, and deterministic validation that each citation resolves to an in-bounds regular text blob in the recorded immutable `$HEAD_REF` before any verdict affects the fix queue.
...
deduplicate normalized Critical/Important findings and assign f001, f002, ...
for each recorded candidate:
    spawn(finding-verifier)  # Stage 3; one candidate per task, at most 3 active
require exact candidate-ID result set; apply confirmed/refuted/needs-verification
...
| `finding-verifier` | Stage 3: once per normalized Critical/Important candidate; evidence verification only |
...
| User's chezmoi (`finding-verifier`) | MIT (umbrella repo license) | n/a | First-party header; no upstream source or bundled license duplication |
...
| 4. Orchestrator | T8 | Write SKILL.md with preconditions, applicability logic, and Stage-1/2/3 gating |
...
- [x] `tests/codex/verify_pr_review_bundle.py` verifies the 8 vendored TOML files declare Apache-2.0 or MIT as expected and reference the expected bundled LICENSE/NOTICE files; it separately pins the first-party verifier's MIT header and read-only inherited-model contract
...
- [x] **V2 abnormal-path executable contract coverage**: the distributed Skill reads pinned scheduler contract V4; `tests/codex/verify_pr_review_bundle.py` rejects drift, and `tests/codex/test_pr_review_v2_scheduler.py` consumes the same file while parameterizing Stage 1/2/3 deadlines and exercising completed/retirement-first delivery inside and at the 60-second boundary, observed-running and valid-final retirement paths, full-tree requirements, parent interruption, identical/conflicting finals before aggregation, later error/interrupted evidence, cleanup monotonicity, canonical sender matching, spawn reconciliation, unexpected descendants, owned cleanup, exact-role/candidate aggregation, and partial-aggregation rejection.
...
- [x] **Issue #282 one-shot comparison retained**: `pr_review_finding_verifier_post_v1.json` records all six historical cases, exact candidate/verdict totals, aggregate usage, abort status, unverified model identity, and the non-causal conclusion. `test_pr_review_finding_verifier_comparison.py` rejects case loss, scoring an aborted case, aggregate drift, or a false claim of demonstrated improvement.
- [x] **Stage 3 verdict and V1 lifecycle replay**: executable models cover confirmed/refuted/needs-verification application, Critical downgrade, Refuted Findings visibility, Important top-five budgeting, empty and partial result sets, V1 returned-agent identity, exact targeted waits, close-before-refill, maximum concurrency, deadline/error cleanup, and exact candidate completeness. These models pin externally observable decisions; they do not claim to replace live runtime smoke.
- [x] **Immutable evidence citation validation**: the bundled validator uses literal Git tree lookup with replacement objects disabled, accepts only regular text blobs at the recorded `$HEAD_REF`, bounds lines/ranges against immutable blob contents, and emits an exact candidate/head/count sentinel. CI covers valid regular/executable files and rejects nonexistent, worktree-only, symlink/tree, binary, empty, control-character, and out-of-bounds evidence before verdict application.
...
Current verification boundary: CI checks bundle integrity and static contract drift via `tests/codex/verify_pr_review_bundle.py`, proves selected high-risk verifier regressions fail closed with `tests/codex/test_verify_pr_review_bundle_negative.py`, validates the accepted historical baseline, sanitized post-change comparison, finding-verdict contract, and immutable evidence resolver, replays PR and `--allow-no-pr` base-resolution command/control-plane decisions with `tests/codex/test_pr_review_base_resolution.py`, and executes the V1 Stage-3 lifecycle plus the V2 abnormal-path state-machine and its retained-list and scheduler-owned dispatch replays with `tests/codex/test_pr_review_v2_scheduler.py`. Static checks cover the three `gpt-5.6-sol` review profiles, both runtime schemas, all pinned machine-readable contracts, agent control-plane bans, first-party read-only inheritance, severity normalization, `$HEAD_REF`-bound collection, authoritative packet hashing, exact candidate identity, evidence value and immutable blob constraints, immutable historical artifact identity, per-case verdict accounting, and negative mutations for high-risk drift. Deterministic replay covers scoped escalation, Stage 1/2/3 lifecycle boundaries, scheduler refill, cleanup ownership, exact-role/candidate completeness, verdict shape, evidence resolution, verdict application, output budgeting, and aggregation decisions. CI does **not** prove Codex CLI profile layering or run live model requests; isolated manual smoke records the effective turn context and actual tool calls without retaining Raw JSONL.
...
| **V2 retained-list rollover or first-snapshot completion hides terminal status** — final delivery, slot refill, and status retention can race; requiring explicit completed or prior-running status alone creates false-negative gates | Low–Medium | Contract V4 retains canonical FINAL plus completed status or qualified retirement after prior running or a validated canonical FINAL. It preserves strict full-tree, 60-second/stage boundaries, parent-interrupt and run-fatal exclusions, ordered reconciliation, late-conflict validation, and exact role/candidate aggregation. Sanitized retention and scheduler-owned replays pin both sequences |
...
| **Orchestrator or verifier bug → false-green or false-red** — candidate extraction, identity matching, fabricated citations, or verdict application could hide a valid blocker or retain a false positive | Low–Medium | The accepted historical fixtures preserve pre/post observations and evaluated artifact hashes without inventing a hard recall threshold or coupling them to the mutable checkout. Static and executable tests pin deterministic IDs, one-candidate scope, typed evidence values, literal immutable-tree resolution, regular-text/blob line bounds, exact result/verdict accounting, verdict application, visible refutations, Critical downgrade, output caps, and V1/V2 lifecycle rules; post-change comparison remains observational because effective model identity was not exposed in every run |
| **Per-candidate Stage 3 cost exceeds its filtering value** | Medium | The first one-shot comparison spent 16,675,099 input tokens across six cases and refuted 1 of 21 candidates. Keep that result visible in the Draft PR, preserve inherited model/effort rather than silently changing quality, and require human acceptance before merge. Future optimization should use a separate measured change rather than weakening Issue #282's explicit Critical/Important coverage contract in this PR |

## References

- [Issue #282](https://github.com/toku345/dotfiles/issues/282) — finding verification and historical comparison
