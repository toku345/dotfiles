...
3. macOS 専用設定 (`karabiner`, `iterm2`, `cmux`, `ghostty`, `google_ime`, `asdf`, fish 関連) を `.chezmoiignore` でスキップ
...
- `~/.config/fish`, `~/.config/karabiner`, `~/.config/iterm2`, `~/.config/asdf`, `~/.local/bin/ghostty-theme*` は**生成されない**
...
Linux 側の starship プロンプトは **Bracketed Segments preset** (ASCII のみ) を使っているため、Nerd Font なしでも正しく描画されます。接続元ターミナル (macOS 側の Ghostty 等) の Nerd Font は Linux 側の表示に影響しません。
