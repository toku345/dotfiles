...
| `CP2_READY` | Evaluator success normally auto-clears the goal; map that event to `CP2_READY_WAIT` in operator state and freeze report/diff/Evidence snapshots |
| Redirect | Create a new contract, run id, and fresh session; do not replace the active goal in place |
| Restart/resume | Treat a resumed active goal as a new run because documented turn/timer/token baselines reset; preserve the earlier run's evidence and cumulative task accounting |
| Terminal human disposition | Confirm no goal remains before ordinary delivery outside Week 0 |

The evaluator's success means only that the CP2-ready handoff was surfaced in the conversation. It is not Acceptance-Criterion verification or human disposition. Do not return Session B to a frozen report; post-freeze fixes require a new run.
