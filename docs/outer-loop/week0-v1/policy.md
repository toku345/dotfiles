...
The cohort `advancement_gate` MUST be derived from exactly four terminal fixed-slot task rollups and the predicates above. Its result is `pass` only when all four hard gates are clear, at least three derived task comfort results are qualifying, and both runtime ship-coverage predicates are true. A human MAY choose `advance` only when that derived gate passes; a passing gate does not prevent `revise-and-rerun` or `stop`. Any contradictory count, boolean, task result, or decision invalidates the cohort record and blocks transfer or advancement.
