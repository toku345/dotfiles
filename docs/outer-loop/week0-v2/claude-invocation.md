...
- Verify the current `B-implementation` enforcement profile/config digest, canonical lexical/resolved roots, task-surface and enforced-denial inventories, collector identity/configuration, passing role preflight, all-role hard-link-creation denial, writable-path symlink/hard-link controls, protected-exclusion/write-root overlap control, credential/environment/socket exposure, network mode and allowlist digests when enabled, host-brokered external-tool inventory plus operation/scope-selector/argument-shape allowlist digests, and passing-control id.
- Confirm only approved target-worktree paths, declared disposable paths, and `report.md` are writable.
- Run the exact operator collector before launch; any incomplete scan, multiply linked readable file, unscannable-readable surface, identity mismatch, or failed role control means no launch.
...
| `CP2_READY` | Evaluator success normally auto-clears the goal; map that event to `CP2_READY_WAIT`, expire authority, prove complete execution-group quiescence, and run the identical postflight collector; only a complete passing role gate may freeze the canonical snapshot and proceed |
| Runtime/host interruption before either marker | Because documented resume resets counters, record `INTERRUPTED_NO_MARKER`, freeze only observed partial evidence with explicit absent/`UNVERIFIED` fields, start no Session C, and allow an unused next attempt only after role mutation is proven stopped and required post-run reconciliation reaches `reconciled-clear` |
