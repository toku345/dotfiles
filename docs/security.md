...
Committed so routine OS-package and runtime updates are deliberate, not implicit. Homebrew policy is canonical in `~/.homebrew/brew.env`; the run-once bootstrap exports the subset that is safe before the managed file and GitHub CLI authentication are available. The asdf config remains OS-agnostic and shell-scoped.

| Location | Setting | Effect |
| --- | --- | --- |
| `~/.homebrew/brew.env` | `HOMEBREW_NO_AUTO_UPDATE=1` | No implicit `brew update` on install — installing a formula no longer silently pulls fresh metadata for everything. |
...
| same | `HOMEBREW_CASK_OPTS=--require-sha` | Refuse casks without a checksum (macOS-relevant; inert on Linux). Checksum-free software is reviewed as a separate vendor distribution rather than installed by bypassing this setting. |
| same | `HOMEBREW_UPDATE_TO_TAG=1` | Keep Homebrew itself on the latest stable tag even if a developer command has enabled developer mode. |
| same | `HOMEBREW_VERIFY_ATTESTATIONS=1` | Verify `homebrew/core` Bottle build provenance with an authenticated GitHub CLI. This does not cover casks, third-party taps, or source builds. |
...
**Note (ask mode):** plan visibility for install/upgrade/reinstall is Homebrew's current default. `HOMEBREW_ASK=1` is a deprecated compatibility variable and must not be exported; `HOMEBREW_NO_ASK` is intentionally not set. The prompt is skipped without a TTY, so this is plan visibility, not an enforcement gate.
...
**Scope limitation (Homebrew):** Homebrew reads the user environment file for invocations that resolve the managed home, including interactive and non-interactive shells. An invocation that changes `HOME`/`XDG_CONFIG_HOME`, runs as another user, or cannot read the managed file does not receive these controls. Homebrew itself rejects normal package operations as root. The run-once bootstrap uses its explicit safe subset before the managed file is applied.
...
The reviewed Homebrew helpers can discover and remember smoke commands. Automatic selection is bound to the named installation rather than an arbitrary same-name executable on PATH, and that binding is rechecked after upgrade. Manual checks remain user-selected commands. Pre-update probes run without stdin, with a ten-second deadline and one-second termination grace for their process group; successful probes are remembered even if the upgrade is subsequently declined or blocked. Per-package settings are local, private JSON files; writes reread the current file under a per-package lock and replace only the requested field. See the [helper runbook](homebrew-update.md) for settings and recovery.
...
The two reviewed Homebrew helpers default to a 48-hour cooldown, with explicit per-package settings (including zero hours). Other routine update channels retain their existing 7-day policy. A zero-hour helper setting waives age waiting only; unverifiable release metadata still requires a reasoned exception, and integrity failures remain fatal. The cooldown must not delay security fixes. Apply an update immediately (skip the cooldown) when:
