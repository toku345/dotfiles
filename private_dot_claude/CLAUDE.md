...
- Claude Code sandbox 内でコマンドが permission / TLS / Mach service 系エラーで落ちたら、コマンドの誤りでなく sandbox 制約を疑い、その 1 回に限り `dangerouslyDisableSandbox` 付き再実行を検討する (恒久 allowlist 化はしない)
...
この「事実で検証する」原則はコードレビューに限らない。ローカルの tool / CLI / 設定の挙動を「推奨」「公式」として提示する前に live tool を叩いて確認する (`--help` / `--version` / 実際の設定出力)。自分で列挙・推測したファイル名群を Read する前に、まずディレクトリを列挙 (ls / glob) し、存在未確認のパスを盲目的に Read しない (ユーザー指定・既出の grep/glob 結果・repo guidance / manifest / tool default で既に名前が出ているパスは対象外)。
