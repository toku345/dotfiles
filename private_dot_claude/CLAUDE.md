...
- リポジトリの `AGENTS.md` / `CLAUDE.md` / `CONTRIBUTING.md` 等に固有規約がある場合は、prefix や ticket ID の要件を含めてそちらを優先する

タイトルで変更内容を検索可能にし、本文にはコードや diff から確定できない理由・制約・意図だけを書く

- 件名は何を変更したかを簡潔に表す
- 変更理由が件名と diff から明らかな場合は、説明本文を付けない
- 半年後の読者が変更の必要性を件名と diff から復元できない場合は、理由を 1-3 文で説明する
- 本文では、AI がコードや diff から確定できない「なぜ」を優先する。原因、回避する問題・リスク、維持すべき制約、重要なトレードオフ、将来の判断に必要な意図的な対象外事項だけを残す
- diff を読めば分かる実装内容の列挙、変更ファイルの一覧、実装経緯、検証ログ、レビュー回数は書かない。必要に応じて PR 本文または最終報告へ分離する
- 性質の異なる変更は長い本文で一つにまとめず、意味のある単位でコミットを分ける
- 本文は必要最低限とし、長さそのものを品質基準にしない
