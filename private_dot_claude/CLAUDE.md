...
- **headless 起動 (`claude -p`) / slash command 経由**: 質問で停止せず、slash command の指示文に含まれる Goal/Constraints/AC を採用して即着手する（`~/.local/bin/triple-review` 等の並列 aggregation 経路の汚染を避けるため）
