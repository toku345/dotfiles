...
コマンド例では環境変数 `CODEX_MODEL` を使用する。未設定時は `gpt-5.3-codex` がデフォルト値となる。利用可能なモデルは [Codex Models](https://developers.openai.com/codex/models/) を参照。
...
```bash
codex exec -m "${CODEX_MODEL:-gpt-5.3-codex}" "以下の実装プランをレビューしてください。致命的な点のみ指摘してください。$(cat {プランファイルパス})"
```

2. 指摘があればプランを修正し、再レビューを依頼する:

```bash
codex exec resume --last -m "${CODEX_MODEL:-gpt-5.3-codex}" "指摘を反映しました。再度レビューしてください。致命的な点のみ指摘してください。$(cat {プランファイルパス})"
```
...
```bash
codex exec -m "${CODEX_MODEL:-gpt-5.3-codex}" "以下の diff をレビューしてください。致命的な点のみ指摘してください。$(git diff {対象})"
```

diff が大きく `$(...)` でのインライン展開が困難な場合は、`-` で stdin からプロンプトを読み取る:

```bash
{ echo "以下の diff をレビューしてください。致命的な点のみ指摘してください。"; git diff {対象}; } | codex exec -m "${CODEX_MODEL:-gpt-5.3-codex}" -
```
