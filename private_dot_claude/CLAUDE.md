...
- **headless 起動 (`claude -p`)**: 対話チャネルが存在しないため、停止して質問することはできない。piped prompt または slash command 指示文をそのまま仕様として扱い、仮定を出力冒頭に宣言してから自律実行する。`~/.local/bin/triple-review` の並列 headless aggregation 経路（内部で `claude -p` を spawn）もこの規定に該当する。**ただし strict envelope を要求する経路** (例: 「いきなり `### X` から始めてください」、JSON / YAML / 特定セパレータ envelope 等の機械パース対象出力) **では仮定宣言も省略し、要求された生フォーマットのみを返す** (envelope 破壊を避けるため)
