...
**auto-mode active 単独では本ファイルの「曖昧さは質問で埋める」を上書きしない**。auto-mode の harness 命令 ("Make reasonable assumptions and proceed") は介入頻度を下げたい希望のシグナルであって、判断責任の委譲ではない。自律実行が認められるのは、専用 CLAUDE.md を `claude --append-system-prompt-file <file>` で明示注入した場合、または対話チャネル不在の headless 起動 (`claude -p`) の場合のみ (silent stall や aggregation 汚染を回避するため)。

仕組みとして、auto-mode は分類器ベースで tool call を auto-approve する (公式 docs: <https://code.claude.com/docs/en/auto-mode-config>)。本ファイル内の**具体的な指示**は hard override ではなく、**Claude 自身の意思決定と分類器の双方への入力** として作用する (公式 docs: "steers both Claude and the classifier")。したがって本ファイルが指示する「質問で埋める」も分類器側に流れる。
