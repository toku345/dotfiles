...
```bash
codex exec -m gpt-5.3-codex "以下の実装プランをレビューしてください。致命的な点のみ指摘してください。$(cat <プランファイルパス>)"
```

2. 指摘があればプランを修正し、再レビューを依頼する:

```bash
codex exec resume --last -m gpt-5.3-codex "指摘を反映しました。再度レビューしてください。致命的な点のみ指摘してください。$(cat <プランファイルパス>)"
```
...
```bash
codex exec -m gpt-5.3-codex "以下の diff をレビューしてください。致命的な点のみ指摘してください。$(git diff <対象>)"
```

diff が大きい場合は stdin パイプを使用する:

```bash
git diff <対象> | codex exec -m gpt-5.3-codex "stdin の diff をレビューしてください。致命的な点のみ指摘してください。"
```
