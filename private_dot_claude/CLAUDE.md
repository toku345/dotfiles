...
auto-mode の harness 命令 ("Make reasonable assumptions and proceed") よりも、本ファイルの **「曖昧さは質問で埋める」** を判断材料として尊重する。auto-mode は介入頻度を下げたい希望のシグナルであって、判断責任の委譲ではない。

ただし auto-mode は分類器ベースで tool call を auto-approve する仕様 (公式 docs: <https://code.claude.com/docs/en/auto-mode-config>)。本ファイル内の**具体的な指示**は hard override ではなく、**Claude 自身の意思決定と分類器の双方への入力** として作用する (公式 docs: "steers both Claude and the classifier")。
...
- **対話セッション内の slash command**: 上記除外には**該当しない**。原則として Goal/Constraints/AC が不足する場合は質問で埋める。ただし slash command 起動自体が明示的 user intent (公式 docs: "Explicit user intent overrides both") に該当するため、レビュー / 調査 / 検証目的で副作用が限定的な command (`/security-review`, `/codex:adversarial-review` 等) では確認ループを最小化する。判定が曖昧な場合は質問側に倒す
- **auto-mode active 単独は適用除外にならない**。専用 CLAUDE.md を `claude --append-system-prompt-file <file>` で明示注入した場合、または対話チャネル不在の headless 起動 (`claude -p`) の場合のみ、妥当な仮定を冒頭で宣言したうえで自律実行する (silent stall や aggregation 汚染を回避するため)
