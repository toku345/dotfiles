...
AI レビューは「日常の床 / 厚い変更の gate」に分ける。軽く扱ってよい対象では組み込みレビューや短いセルフチェックを使い、重い multi-agent gate は起動しない。厚く理解する対象では `/pr-review-toolkit:review-pr` / `/security-review` / `/codex:adversarial-review` / `triple-review` のような実在する review path を使う。
