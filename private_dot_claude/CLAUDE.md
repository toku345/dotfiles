...
Claude Code sandbox 内でコマンドが permission / TLS / Mach service 系エラーで落ちたら、コマンド自体の誤りより先に sandbox 制約を疑う。ハーネスの手順に従いその 1 回に限り `dangerouslyDisableSandbox` 付き再実行で切り分ける。恒久的な allowlist 追加は独断で行わない (ユーザー判断)。
