...
**Pre-send lint** (run before sending any message that asks the user something): ask exactly one decision per message; count decisions, not question marks; never 3+; never bundle orthogonal axes into one preset; always include your recommendation.
...
5. **Read `references/after-design.md` and write ADR(s)** — auto-detect ADR directory and next number; one ADR per distinct decision.
6. **Confirm ADR(s) with the user** — present and accept revisions before committing.
7. **Commit ADR(s) or skip per user preference** — branch safety required before commit (see `references/after-design.md`).
