...
ただし**ドキュメント拡張子であっても control plane** に該当する変更は Codex 自動レビュー対象とする。control plane の具体定義は user-global CLAUDE.md (`~/.claude/CLAUDE.md`) `## Definition of Done` § 適用除外 に集約 (二重定義回避):

- agent 指示 (`CLAUDE.md` / `AGENTS.md`)
- skill 定義 (`SKILL.md`)
- Claude Code shared settings (`.claude/settings.json` / `~/.claude/settings.json`)。`*.local.json` は per-machine 限定で対象外
- ADR (`docs/adr/**`)
