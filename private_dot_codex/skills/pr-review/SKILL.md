...
6. **Stage 3 — verify normalized Critical/Important candidates**:
   ...
   - Require exactly one usable result for the exact expected candidate-ID set. Each result's first line must match `VERIFICATION_OK finding-verifier <candidate_id> $BASE_COMMIT...$HEAD_REF <packet_sha256>`, followed by a single JSON object matching the contract. Every evidence item must contain exactly a non-empty repository-relative `path`, a positive integer or positive line-range `line`, and a non-empty `observation`; reject absolute/traversal/control-character paths, placeholder values, zero/negative lines, and extra fields. `FATAL_VERIFICATION_ERROR`, unknown/duplicate/missing candidate IDs, scope/hash mismatch, malformed JSON, invalid fields, invalid verdict shape, task failure, timeout, or partial delivery fails closed before aggregation.
   - Before applying any verdict, serialize each parsed result JSON object to a dedicated temp file outside the worktree using a quoted heredoc delimiter, then run the bundled `scripts/validate_finding_evidence.py` with the repository root, exact recorded `$HEAD_REF`, and result-file path. The validator resolves each path as a literal entry in the immutable `$HEAD_REF` Git tree, accepts only regular text blobs, and requires the cited line or inclusive range to fit within that blob's logical line count. Require exit 0 and exact stdout `EVIDENCE_OK finding-verifier <candidate_id> $HEAD_REF <evidence_count>`. A missing script, write or execution failure, nonzero exit, unexpected stdout, absent/non-file/binary path, out-of-bounds line, or evidence-count mismatch is fatal before verdict application. Never validate against the current worktree and never interpolate an evidence path into a shell command.
