...
3. **Await all Stage 1 specialists** before proceeding.
   ...
   - **The V2 adapter** follows this state machine for each canonical task:
     ...
     | retired-seen | task was observed running, then absent from a successful full-tree snapshot, with no final payload | allow the same bounded delivery grace |
     | usable | valid final payload plus completed status or qualified retirement | retain evidence and fill capacity |
     | fatal | error/interrupted status, unobserved disappearance, parent-interrupted retirement, invalid evidence, or expired deadline/grace | clean up and abort |
     ...
     - Treat `completed` as normal lifecycle evidence. As a V2 compatibility fallback, also accept **qualified retirement** only when the exact canonical task was previously observed with `running` status in a successful full-tree snapshot, the parent has never called `interrupt_agent` on it, no run-global fatal evidence exists, and a later successful full-tree snapshot omits it. A spawn result, a failed/incomplete list, mere name matching, or disappearance before observed running is not retirement evidence and is fatal. This fallback relies on the recognized V2 runtime delivering `FINAL_ANSWER` only at child-turn completion; if that runtime semantics or schema is not established, fail closed instead of applying it. Any descendant other than a saved, tracked top-level specialist is an **unexpected descendant** and is fatal.
