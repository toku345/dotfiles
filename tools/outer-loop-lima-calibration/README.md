...
- C03 applies only to agent-launched commands. It pairs an operator-side nonce canary with a guest execution receipt and the actual Codex/Claude tool event. Claude first runs the same command through pinned SRT and then through the real Bash tool; stage two is load-bearing. Every matrix target must appear exactly once as either a passing applicable control or an `UNAVAILABLE_BASELINE` observation; unavailable paths never count as passing denials. Main-process egress remains accepted risk AR-02, outside control aggregation.
