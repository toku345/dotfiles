...
If a bounded post-start command fails, the CLI may add one allowlisted `failure_stage` such as `POST_START_MOUNT_POLICY_CHECK`, `POST_START_HARNESS_SETUP`, or `POST_START_POLICY_CHECK` to its otherwise generic error JSON. The mount policy is checked through one dedicated guest script immediately after harness copy and again during the post-setup policy check, so mount failures are not mislabeled as generic setup failures. The stage is selected only at fixed call sites. Raw argv, stdout, stderr, URLs, guest paths, and exception details are not surfaced or added to runtime state/evidence.
