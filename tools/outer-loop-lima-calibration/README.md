...
After that copy is validated, `preflight` checks the reviewed policy subset and pinned artifact versions in the frozen Codex `0.144.5` lock and seeds before H1 or any guest creation. The normalized requirements must allow exactly approval policy `never`, sandbox modes `read-only` and `workspace-write`, and web-search mode `disabled`; the active config must select `never`, `workspace-write`, and `disabled`. This version-specific static check is an early rejection boundary, not a complete proof of upstream policy semantics. The authoritative live check remains the pinned Codex binary's post-start `app-server --strict-config` introspection, executed as the non-sudo `calibration` user.

The profiles are static YAML. They use VZ/aarch64 with 4 CPUs, 8 GiB RAM, and a 40 GiB disk, and disable configured host mounts, additional disks, containerd, declared networks, port forwarding, proxy propagation, SSH-agent forwarding, and X11 forwarding. Lima's exact internal `/mnt/lima-cidata` mount is permitted; every other `/mnt/lima-*` target and every host-style `/Users`, `/Volumes`, or provisioner-share target is rejected. Failure to enumerate the mount table also blocks. Provisioning uses only the frozen harness and creates the non-sudo `calibration` account. Authentication remains guest-local.
...
The operational logical state root is `~/.local/state/outer-loop/lima-prearm/v1`. Each run binds write-once to a fresh short physical `LIMA_HOME` at `~/.local/state/ol/<10-char-token>`, where the lowercase Base32 token is deterministically derived from a domain-separated SHA-256 of the canonical state root and run ID. The harness checks both its `<=95`-byte socket-path policy and Lima `2.2.0`'s `<104`-byte limit before its first write. It then creates the logical run tree before allocating the physical home, so a logical-state creation failure cannot consume a pool token. Existing bindings, token directories, instance directories, disks, symlink ancestry, or ambiguous Lima state are never reused. A crash or write failure during the later physical allocation can still leave an orphan home or binding; Week 0 accepts that fail-closed risk and requires operator inspection rather than automatic rollback or reuse.
...
Instance names are fixed as `outer-loop-week0-codex` and `outer-loop-week0-claude`. Preflight parses Lima `2.2.0` list output as JSON Lines and requires an empty namespace plus both fixed instance-directory/root-disk paths absent. Arrays, malformed or mistyped records, `Broken` instance status, nonzero results, mixed/additional stderr, and any non-canonical empty result are `UNKNOWN`. H1 binds the physical-home identity, harness manifest, parser contract, and absence snapshot. After retention registration, provision repeats that freshness check immediately before any create, then performs `create -> Stopped identity` for both guests before `start -> Running identity`. Create/start attempts are append-only and are written before invocation. An orphan, duplicate, contradiction, or identity drift blocks without same-run retry.

The Lima `2.2.0` parser contract and refreshed host lock intentionally produce new identities. A run initialized or preflighted under an earlier Lima/Python baseline cannot continue across this change; after merge and static receipt, live calibration must use a new run ID.

If a bounded post-start command fails, the CLI may add one allowlisted `failure_stage` such as `POST_START_MOUNT_POLICY_CHECK`, `POST_START_HARNESS_SETUP`, `POST_START_POLICY_CHECK`, `POST_START_RUNTIME_VERSION_CHECK`, or `POST_START_RUNTIME_POLICY_CHECK` to its otherwise generic error JSON. Common privilege and mount policy, exact runtime versions, and runtime-specific effective policy are separate fixed call sites, so a failure can be classified without surfacing untrusted command details. The mount policy is checked through one dedicated guest script immediately after harness copy and again during the common post-setup policy check. Raw argv, stdout, stderr, URLs, guest paths, and exception details are not surfaced or added to runtime state/evidence.
...
- C00 validates the host lock/manifest, both guest/runtime identities, root-owned policy, and Codex's key-by-key `config/read` plus `configRequirements/read` effective values. The stdio client waits for each request result before advancing the initialize/read sequence; list-valued config is accepted only when every indexed origin is present and resolves to the same expected system policy file.
- C01 proves distinct instance/disk identities and the non-sudo, no-host-share, runtime-non-writable policy boundary; exact Lima-internal `/mnt/lima-cidata` is the sole `/mnt/lima-*` exception.
...
Deadline, abandonment, exposure, or cohort completion blocks new work. Cleanup first serializes behind any current run operation and reloads committed state. A fixed guest is eligible for best-effort Lima stop/delete only when every recorded provision attempt has terminal outcome `SUCCESS`, the attempt sequence is complete, and its strict live identity matches the recorded Stopped or Running identity. Any `NONZERO`, `TIMEOUT`, `UNAVAILABLE`, `IDENTITY_MISMATCH`, orphan, duplicate, or contradictory provision outcome makes automatic guest cleanup ineligible and transitions directly to `CLEANUP_MANUAL_REQUIRED` without stop/delete.

For a fully successful provision history, a recognized Running guest may be stopped by fixed name, its Stopped identity read back, and the recognized fixed guest deleted through Lima. Each Lima call has an immediate strict identity recheck. The harness does not directly delete any guest or Lima administrative filesystem content. Lima normally leaves administrative entries in the physical home, and any such residue transitions to `CLEANUP_MANUAL_REQUIRED` without `unlink`, recursive deletion, or another filesystem cleanup attempt.

Automatic `CLEANUP_VERIFIED` is intentionally narrower: it is available only when provisioning never started, strict instance/disk absence is proved, and the bound physical home has remained completely empty. In that case the only direct filesystem removal is one identity-checked `rmdir` of that empty home. Every `UNKNOWN`, partial directory, unrelated or administrative residue, identity/binding drift, non-success provision outcome, failed Lima call, or residual content records `CLEANUP_MANUAL_REQUIRED` with no further destructive action.

Transitioning to `CLEANUP_MANUAL_REQUIRED` also attempts to disable the retention job. Failure to disable it is recorded, but the persisted manual disposition and same-run cleanup guard still prevent an hourly catch-up from retrying destructive cleanup. Retention therefore guarantees neither deletion nor successful job removal by the deadline; a guest, disk, administrative entry, or home may remain for operator inspection. Cleanup attestations are separate schema-2 records bound to the immutable calibration seal.

In manual-required state, `status` is the read-only diagnostics surface. It reports an allowlisted reason code plus the recorded run ID, logical state/run roots, pool/home, fixed guest names, instance-directory and disk paths, binding registry, and evidence path. It never reports raw Lima output and does not print or execute deletion commands. After operator action outside the harness, read-only `verify-cleanup` reaches `CLEANUP_VERIFIED` only when the strict namespace, fixed directories/disks, physical home, inactive retention job, and any account-revoke disposition are all clear. The wrapper, plist, and evidence remain as operator-owned records. An unresolved manual-required run or unexplained/unbound pool entry blocks another live run.
