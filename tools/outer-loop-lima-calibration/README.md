...
- C03 applies only to agent-launched commands. It pairs an operator-side nonce canary with a guest execution receipt and the actual Codex/Claude tool event. Claude first runs the same command through pinned SRT and then through the real Bash tool; stage two is load-bearing. Unavailable paths are recorded as `UNAVAILABLE_BASELINE`, never as a passing denial. Main-process egress remains accepted risk AR-02, outside control aggregation.
...
After provisioning, the frozen harness generates a per-run LaunchAgent with `RunAtLoad`, deadline calendar scheduling, and hourly catch-up. Its wrapper independently compares the exact immutable UTC deadline and exits without side effects before it is due. The live cycle reads back bootstrap/print and runs a not-due kickstart before authentication.
