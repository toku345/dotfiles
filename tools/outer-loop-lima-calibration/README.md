...
After that copy is validated, `preflight` checks the frozen Codex `0.144.5` lock and seed contract before H1 or any guest creation. The normalized requirements must allow exactly approval policy `never`, sandbox modes `read-only` and `workspace-write`, and web-search mode `disabled`; the active config must select `never`, `workspace-write`, and `disabled`. This version-specific static check is an early rejection boundary, not a complete proof of upstream policy semantics. The authoritative live check remains the pinned Codex binary's post-start `app-server --strict-config` introspection, executed as the non-sudo `calibration` user.
